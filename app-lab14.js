...
angularApp.directive("searchResults", function(){
    ...
    return{
       ...
       // Directive Flow explained
       // 1. Register your directive named as "searchResults" in your angular module in this javascript file
       // 2. This directive has visibility to its parent controller and in this case "mainController"
       // 3. In the "mainCointroller" we have scope and some variables tied to the scope (like person.name, person.address)
       // 4. In your htmp page, "main.html" you can now use the Custom directive "searchResults" written by you as "search-results" (in javascript the directive name will in camelcase, and the same can be referred as lower case/hyphen in the html)
       // 5. When you are using "search-results" in the html, angular will inspect this tag and get the content configured in either "template" or "templateUrl" property.
       // 6. Make sure custom attributes are defined in the custom tags <search-results person-name="{{ person.name }}"></search-results>
